...
def count_occurrences_in_text(word, text):
    ...
    ## remove special chars at begining and end of text and disable ascii characters
    text_encode = text.strip("_'\"").encode("ascii", "ignore")
    text_decode = text_encode.decode()
    ## add space between each special chars and lowercase text and split each words
    t = re.sub('([.,!«»\":?()])', r' \1 ', text_decode)
    t = t.lower().split()
    ## add space between each special chars and lowercase word and split each words
    ...
    ## Browse each words of text
    for i in range(0, t_length): 
        ## check if words is in text
        good_words = 0
        ## Browse each words to find
        for j in range(0, words_length) :
            ## check if expressions is in text
            if ((i+j) < t_length and words[j] == t[i+j]): 
                ## numbers of exact words found
                good_words = good_words +1
        ## if numbers of exact words found, count this occurrence
